...
helper=r'''static void add_x86_runtime_helpers(LinkState&st){
    ...
    // MSVC x86 __allmul. The low 64-bit product is identical for signed and
    // unsigned two's-complement multiplication. Return EDX:EAX, callee pops 16.
    const uint8_t code[]={
        0x53,
        0x8b,0x44,0x24,0x0c,
        0xf7,0x64,0x24,0x10,
...
if old not in s: raise SystemExit('builtin RVA anchor missing in jauld_part03.inc')
s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')
