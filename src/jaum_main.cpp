...
static std::string ext_for(const std::string&type,const std::string&target){bool win=target.rfind("windows-",0)==0;if(type=="exe")return win?".exe":"";if(type=="shared")return win?".dll":".so";if(type=="static")return win?".lib":".a";if(type=="obj")return win?".obj":".o";if(type=="asm")return ".s";return "";}
static int build_one(const fs::path&cfg,const std::string&override_target){auto m=config(cfg);auto get=[&](const std::string&k,const std::string&d=""){auto i=m.find(k);return i==m.end()?d:i->second;};std::string name=get("name","app"),source=get("source","src/main.jau"),type=get("type","exe"),target=override_target.empty()?get("target","linux-x86_64"):override_target,opt=get("optimize","0");if(type=="native")type="exe";if(type!="exe"&&type!="shared"&&type!="static"&&type!="obj"&&type!="asm")throw std::runtime_error("type must be exe, shared, static, obj or asm in JauM 0.9");std::string out=get("output");if(out.empty())out=(fs::path(get("build_dir","build"))/(name+"-"+target+ext_for(type,target))).string();out=replace_all(out,"{name}",name);out=replace_all(out,"{target}",target);out=replace_all(out,"{ext}",ext_for(type,target));if(fs::path(out).has_parent_path())fs::create_directories(fs::path(out).parent_path());auto self=self_path(nullptr);auto jauc=self.parent_path()/
#ifdef _WIN32
"jauc.exe";
#else
"jauc";
